...
val episodes = episodeElements
    ...
    newMovieLoadResponse(title, url, TvType.Movie, episodes.firstOrNull()?.data ?: url) {
        ...
        document.selectFirst("div.player-embed iframe")
            ?.getIframeAttr()
            ?.let { iframe ->
                loadExtractor(httpsify(iframe), data, subtitleCallback, customCallback)
            }
        ...
        for (opt in mirrorOptions) {
            ...
            try {
                ...
                if (!mirrorUrl.isNullOrBlank()) {
                    loadExtractor(httpsify(mirrorUrl), data, subtitleCallback, customCallback)
                }
            } catch (_: Exception) {
        ...
        for (a in downloadLinks) {
            ...
            if (url.isNotBlank()) {
                loadExtractor(httpsify(url), data, subtitleCallback, customCallback)
            }
        }

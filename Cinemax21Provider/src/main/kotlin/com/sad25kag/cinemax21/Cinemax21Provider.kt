...
open class Cinemax21Provider : TmdbProvider() {
    ...
    override suspend fun loadLinks(
        ...
        runAllAsync(
            ...
            {
                invokeVidlink(res.id, res.season, res.episode, countedCallback)
            },
            {
                invokeVidsrccc(
                    res.id,
            ...
            {
                invokeVixsrc(res.id, res.season, res.episode, countedCallback)
            },
            {
                invokeCinemaOS(
                    res.imdbId,
            ...
            {
                if (!res.isAnime) invokeRiveStream(res.id, res.season, res.episode, countedCallback)
            },
            {
                invokeVidsrc(
                    res.imdbId,
            ...
            {
                invokeVidfast(res.id, res.season, res.episode, subtitleCallback, countedCallback)
            },
            {
                invokeMapple(res.id, res.season, res.episode, subtitleCallback, countedCallback)
            },
            {

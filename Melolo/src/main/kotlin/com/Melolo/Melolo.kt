...
class Melolo : MainAPI() {
    // Menggunakan domain melolo.com karena tmthreader.com terbukti NXDOMAIN
    ...
    override suspend fun loadLinks(data: String, isCasting: Boolean, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit): Boolean {
        ...
        val body = """
            {
                ...
                "NovelCommonParam": {
                    "app_language": "id", "sys_language": "id", "user_language": "id", "region": "ID", "time_zone": "Asia/Jakarta"
                }
            }
        """.trimIndent()
        ...
        val responseText = executeWithRetry {
            ...
            // Mengarahkan request API langsung ke domain utama melolo.com
            app.post(
        ...
        val videoUrl = resp?.data?.main_url ?: resp?.data?.video_info?.main_url ?: resp?.data?.video_model?.video_list?.values?.firstOrNull()?.main_url
    ...
    // --- Private Fetchers Tetap via Azure Proxy agar Metadata Aman ---
    ...
    private suspend fun fetchSearch(query: String, limit: Int, offset: Int): List<CatalogBook> = tryParseJson<CatalogSearchResponse>(app.get("$catalogBase/search?query=${URLEncoder.encode(query, "UTF-8")}&limit=$limit&offset=$offset").text)?.data?.search_data?.flatMap { it.books }.orEmpty().filter { it.language.equals("id", true) }
    private suspend fun fetchDetail(bookId: String): CatalogVideoData = tryParseJson<CatalogDetailResponse>(app.get("$catalogBase/detail/$bookId").text)?.data?.video_data ?: throw ErrorLoadingException("Detail Empty")
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        val items = books.mapNotNull { b -> newTvSeriesSearchResponse(b.book_name ?: return@mapNotNull null, "$mainUrl/series/${b.book_id ?: return@mapNotNull null}", TvType.TvSeries) { this.posterUrl = b.thumb_url } }
        return newHomePageResponse(HomePageList(request.name, items), false)
    }

    override suspend fun search(query: String): List<SearchResponse> = fetchSearch(query, 20, 0).mapNotNull { b -> newTvSeriesSearchResponse(b.book_name ?: return@mapNotNull null, "$mainUrl/series/${b.book_id ?: return@mapNotNull null}", TvType.TvSeries) { this.posterUrl = b.thumb_url } }

    // --- Data Classes ---
    data class PlayerVideoModelResponse(@JsonProperty("data") val data: PlayerVideoModelData? = null)
    data class PlayerVideoModelData(@JsonProperty("main_url") val main_url: String? = null, @JsonProperty("backup_url") val backup_url: String? = null, @JsonProperty("video_info") val video_info: PlayerVideoInfo? = null, @JsonProperty("video_model") val video_model: PlayerVideoModel? = null)
